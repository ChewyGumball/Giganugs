...
namespace Giganugs::Resources {
	...
	protected:
		...
		virtual Graphics::Texture* create(const std::string & filename, uint32_t ID) const override;
		virtual bool reload(const std::string & filename, Graphics::Texture& resource) const override;
	};
}
